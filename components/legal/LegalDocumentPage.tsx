    ...
    <section className="section-shell bg-[color:var(--color-background)]">
      <Container>
        <div className="max-w-[62rem]">
          ...
          <p className="mt-4 max-w-[56rem] text-[0.88rem] leading-7 text-[color:var(--color-muted)] sm:text-[0.9rem]">{document.intro}</p>
          ...
          {document.pendingItems?.length ? (
            <article className="mt-4 rounded-[1rem] border border-[color:var(--color-brand)]/25 bg-[color:var(--color-brand-soft)]/58 px-4 py-3.5">
              <p className="text-[0.66rem] font-semibold uppercase tracking-[0.1em] text-[color:var(--color-brand)]">
                Campos pendientes de cierre legal
              </p>
              ...
              {section.bullets?.length ? (
                <ul className="mt-3 space-y-2">
                  {section.bullets.map((bullet) => (
                    <li key={bullet} className="relative pl-4 text-[0.84rem] leading-6 text-[color:var(--color-muted)] before:absolute before:left-0 before:top-[0.7rem] before:h-1.5 before:w-1.5 before:rounded-full before:bg-[color:var(--color-accent)]">
                      {bullet}
                    </li>
                  ))}
                </ul>
