...
export default async function FloatingActions() {
  ...
  return (
    <div className="pointer-events-none fixed bottom-4 right-4 z-40 md:hidden">
      <Link
        href={whatsappHref}
        target="_blank"
        rel="noreferrer"
        className="pointer-events-auto inline-flex h-[2.95rem] items-center justify-center rounded-full border border-[color:var(--color-line)] bg-[color:var(--color-surface-strong)] px-4 text-[0.62rem] font-semibold uppercase tracking-[0.08em] text-[color:var(--color-foreground)] shadow-[0_20px_44px_-30px_rgba(42,20,31,0.62)] transition-[border-color,color,transform] duration-300 hover:-translate-y-px hover:border-[color:var(--color-brand)] hover:text-[color:var(--color-brand)] md:h-10 md:px-5 md:text-[0.66rem]"
